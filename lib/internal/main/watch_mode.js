...
const kShouldFilterModules = (getOptionValue('--watch-path').length + getOptionValue('--watch-glob').length) === 0;
const kEnvFile = getOptionValue('--env-file');
const kWatchedPaths = ArrayPrototypeMap(getOptionValue('--watch-path'), (path) => resolve(path))
ArrayPrototypePushApply(kWatchedPaths, ArrayPrototypeFlatMap(getOptionValue('--watch-glob'), (path) => globSync(path)));
