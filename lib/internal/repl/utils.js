...
const { tokTypes: tt, Parser: AcornParser, parse: acornParse } =
      ...
      session.post('Runtime.evaluate', {
        ...
        // may be inspected.
          ...
          callback(null, null);
                   ...
                   StringPrototypeSlice(result.description, -17) !== '{ [native code] }') {
          const ast = acornParse(`${input} = ${result.description}`, { ecmaVersion: 2020 });
          const args = ArrayPrototypeMap(ast.body[0].expression.right.params, ({ name }) => name);
          callback(null, `${input}(${ArrayPrototypeJoin(args, ', ')})`);
