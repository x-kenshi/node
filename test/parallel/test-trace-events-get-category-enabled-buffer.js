...
it('should track enabled/disabled categories', () => {
  ...
  assert.strictEqual(buffer[0], 0, `the buffer[0] should start with value 0, got: ${buffer[0]}`);

  tracing.enable();

  let currentCategories = getEnabledCategories();

  assert.includes(currentCategories, category, `the getEnabledCategories should include ${category}, got: ${currentCategories}`);
  assert.ok(buffer[0] > 0, `the buffer[0] should be greater than 0, got: ${buffer[0]}`);

  tracing.disable();

  currentCategories = getEnabledCategories();
  assert.strictEqual(currentCategories, undefined, `the getEnabledCategories should return undefined, got: ${currentCategories}`);
  assert.strictEqual(buffer[0], 0, `the buffer[0] should be 0, got: ${buffer[0]}`);
});
