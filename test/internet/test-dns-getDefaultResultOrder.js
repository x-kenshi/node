...
assert.strictEqual(dnsOrder, 'ipv4first');
dns.setDefaultResultOrder('ipv6first');
dnsOrder = dns.getDefaultResultOrder();
assert.strictEqual(dnsOrder, 'ipv6first');
dns.setDefaultResultOrder('verbatim');
dnsOrder = dns.getDefaultResultOrder();
assert.strictEqual(dnsOrder, 'verbatim');

{
  (async function() {
    ...
    assert.strictEqual(result.address, result1.address);
    assert.strictEqual(result.family, result1.family);
